...
export default L.Map.AstroMap = L.Map.extend({
    ...
    loadLayerCollection: function (name) {
        if (this.layers.hasOwnProperty(name)) this.layers[name].addTo(this)
        else {
            const bestName = Object.keys(this.layers)[0]
            if (bestName) this.layers[bestName].addTo(this)
        }
    },
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            if (currentTarget['name'].toLowerCase() == this._target.toLowerCase()) {
                ...
                for (let j = 0; j < jsonLayers.length; j++) {
                    ...
                    if (currentLayer['projection'].toLowerCase() != projection.toLowerCase()) {
                        continue
                    }
