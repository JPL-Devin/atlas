...
const ProductIconsRoot = styled('div', {
    shouldForwardProp: (prop) => prop !== 'isSmall' && prop !== 'isDark' && prop !== 'isMissing',
})(({ theme, isSmall, isDark, isMissing }) => ({
    color: 'white',
    opacity: 0.7,
    ...(isSmall && {
        transform: 'scale(0.4)',
    ...
    if (type) {
        switch (type) {
            ...
            case 'volume':
                ...
                break
            case 'directory':
                Icon = <FolderIcon style={defaultIconStyle} />
                break
            case 'file':
                Icon = <InsertDriveFileOutlinedIcon style={defaultIconStyle} />
                break
            default:
                isMissing = true
                Icon = <ImageIcon style={defaultIconStyle} />
        }
    } else {
