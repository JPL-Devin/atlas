...
const GridCard = ({ index, data, width }) => {
    ...
    // Detect if this directory is a bundle/volume (single dir in relative path)
    // Pattern: atlas:{standard}:{mission}:{spacecraft}:/bundle_name/
    const isBundleOrVolume = (uri) => {
        if (!uri) return false
        // Get the relative path part (after the last ':' before '/')
        const parts = uri.split(':')
        if (parts.length < 2) return false
        const relativePath = parts[parts.length - 1]
        // Check if it's a single directory: /dirname/ with no additional slashes
        const match = relativePath.match(/^\/([^\/]+)\/$/)
        return match !== null
    }

    const getBundleVolumeName = (uri) => {
        if (!uri) return null
        const parts = uri.split(':')
